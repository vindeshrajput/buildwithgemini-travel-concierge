...
def _ensure_begin_rendering(messages: list[dict]) -> None:
    """Prepend a beginRendering message if missing from surfaceUpdate messages."""
    has_br = any("beginRendering" in m for m in messages)
    if has_br:
        return
    ...
    for m in messages:
        su = m.get("surfaceUpdate")
        if isinstance(su, dict) and su.get("components"):
            surface_id = su.get("surfaceId", "main")
            ...
            for c in components:
                if isinstance(c, dict) and c.get("id") == "root":
                    root_id = "root"
                    break
            if not root_id and components and isinstance(components[0], dict) and "id" in components[0]:
...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        _ensure_begin_rendering(messages)
